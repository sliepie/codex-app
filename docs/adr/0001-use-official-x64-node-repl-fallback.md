...
Use the latest official closed-source x64 `node_repl.exe`, Chrome `extension-host.exe`, and Computer Use `codex-computer-use.exe` as committed vendored fallbacks until Windows ARM64-native equivalents can be compiled, downloaded, or otherwise obtained.
...
- Future work should remove these exceptions when ARM64-native `node_repl.exe`, `extension-host.exe`, `codex-computer-use.exe`, and `tectonic.exe` binaries become available.
