...
Hydration may install or upgrade the official Microsoft Store Codex app (`9PLM9XGG6VKS`), copy `node_repl.exe` from the installed package location, and uninstall Codex afterward only if the script installed it into a previously missing state.
