...
test("Codex app UI override and Windows menu-bar tweak install independently", () => {
  ...
  try {
    ...
    assert.ok(
      uiOverrideCss.includes(
        String.raw`:where(aside,nav,[role="navigation"]):has([data-app-action-sidebar-section-heading]) [class~="group/chats-section-header"]>[class~='flex'][class~='items-center'][class~='gap-1']{translate:7px 0!important;}`,
      ),
    );
    assert.ok(
      uiOverrideCss.includes(
    ...
    assert.ok(
      uiOverrideCss.includes(
        String.raw`[data-radix-popper-content-wrapper]:has(:where([role='menu']) [role='menuitem'] svg path[d^='M10.6391 1.67517']):has(:where([role='menu']) .flex.flex-col.text-sm>.grid.items-center.gap-y-1\.5.py-1){translate:0 -58px!important;}`,
      ),
    );
    assert.ok(
      uiOverrideCss.includes(
