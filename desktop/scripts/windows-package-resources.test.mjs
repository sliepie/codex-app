  ...
  try {
    ...
    assert.ok(
      uiOverrideCss.includes(
        String.raw`:where(aside,nav,[role="navigation"]):has([data-app-action-sidebar-section-heading])>[class~='relative'][class~='z-10'][class~='shrink-0'][class~='flex-col'][class~='gap-2'][class~='px-row-x'] :is(button,div)[class~='relative'][class~='h-[var(--height-token-row)]'][class~='py-row-y'],:where(aside,nav,[role="navigation"]):has([data-app-action-sidebar-section-heading]) [data-app-action-sidebar-scroll]>[class~='flex'][class~='shrink-0'][class~='flex-col'][class~='gap-2'] :is(button,div)[class~='relative'][class~='h-[var(--height-token-row)]'][class~='py-row-y']{height:calc(var(--height-token-row) - 4px)!important;}`,
      ),
    );
    assert.match(
    ...
    assert.ok(
      uiOverrideCss.includes(
        String.raw`:where(aside,nav,[role="navigation"]):has([data-app-action-sidebar-section-heading]) [data-app-action-sidebar-project-row] [data-sidebar-project-drop-zone='project-icon'] > :first-child{translate:-1px 0!important;}`,
      ),
    );
    assert.ok(
      uiOverrideCss.includes(
    ...
    assert.ok(
      uiOverrideCss.includes(
        String.raw`:where(aside,nav,[role="navigation"]):has([data-app-action-sidebar-section-heading]) :is([data-app-action-sidebar-section-heading="Projects"],[data-app-action-sidebar-section-heading="Pinned"]) [data-app-action-sidebar-section-toggle]{translate:-1px 0!important;}`,
      ),
    );
    assert.ok(
      uiOverrideCss.includes(
    ...
    const usageLinkHideRule = uiCssRules.find(
      ({ selector, declarations }) =>
        ...
        selector.includes('a[href*="highlight_plan="][href$="#pricing"]'),
    );
    assert.ok(usageLinkHideRule);
    assert.deepEqual(usageLinkHideRule.selector.split(","), [
      String.raw`.flex.flex-col.text-sm:has(>.grid.items-center.gap-y-1\.5.py-1)>a[href*="highlight_plan="][href$="#pricing"]`,
