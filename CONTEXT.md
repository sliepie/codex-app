...
- The Windows package uses the **Electron host** with Codex++ restored: the packaged app starts through `codex-plusplus/loader.cjs`, hydrates Codex++ runtime assets, applies the self-signed bundle rewrite and Windows primary taskbar-window patch, exposes Codex window services to the Codex++ runtime, and removes the unsupported Codex Micro / Work Louder device integration. Upstream owns OWL and message-rail behavior; the package does not rewrite either feature.
