...
- Electron testbed: the default package should keep the Electron-produced root executable and `ffmpeg.dll`, should not contain Store/Owl `chrome.dll` or root `owl-shell-runtime.json`, should not hydrate Codex++, should not apply recovered-source feature patches, and should set the packaged app `main` to the recovered upstream Codex main rather than `codex-plusplus/loader.cjs`.
