...
description: "Store package update for codex-app. Use when refreshing Store-vendored helper binaries, maintaining the parked Store/Owl Codex.exe and chrome.dll payload, or changing Store provenance and resource-binary exceptions."
---
...
- Goal: keep the Windows package as a clean Forge/Electron testbed first. Store/Owl shell staging is parked in source and not wired into the default Windows package.
- Customization order: validate the clean Electron host first; then bring back one customization per follow-up commit when there is a concrete reason to test it.
- Electron testbed: the default package should keep the Electron-produced root executable and `ffmpeg.dll`, should not contain Store/Owl `chrome.dll` or root `owl-shell-runtime.json`, should not hydrate Codex++, should apply only the Windows primary taskbar-window recovered-source patch, and should set the packaged app `main` to the recovered upstream Codex main rather than `codex-plusplus/loader.cjs`.
- Store/Owl goal: keep the official Store/Owl shell payload available for later isolated experiments. It is not a completed Windows package change until a later PR deliberately wires it back in.
...
- Package staging: do not wire Store/Owl payload staging into the clean Electron testbed. Updating metadata or payload automation alone is not a completed Windows package change.
...
- If the clean Electron host fails a specific behavior check, bring back exactly one customization in a follow-up commit and validate it against that behavior.
