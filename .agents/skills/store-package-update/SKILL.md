...
- Package staging: wire the Windows package flow to stage the tracked Store/Owl payload into the built MSIX/AppX payload before validation. Updating metadata or payload automation alone is not a completed Store/Owl shell change. Completion criterion: package-resource tests cover the package staging path and the build fails when the tracked payload is missing.
