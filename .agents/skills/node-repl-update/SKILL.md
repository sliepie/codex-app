...
- Refresh `node_repl.exe` only from the official Microsoft Store Codex app (`9PLM9XGG6VKS`) through `npm run update:node-repl`.
- Do not replace the vendored binary from an arbitrary local path.
